...
/* TX RADIO sx1276 */
radiodev *sxradio;
static bool sx1276_tx = false;
static uint8_t sx1276_pw = 20;
...
static int parse_gateway_configuration(const char * conf_file) {
    ...
    /* if sx1276 enable */
    val = json_object_get_value(conf_obj, "sx1276_tx");
    if (json_value_get_type(val) == JSONBoolean) {
        sx1276_tx = (bool)json_value_get_boolean(val);
    }
    MSG_DEBUG(DEBUG_INFO, "INFO~ packets transmit using sx1276 will%s be enable\n", (sx1276_tx ? "" : " NOT"));

    val = json_object_get_value(conf_obj, "sx1276_pw");
    if (val != NULL) {
        sx1276_pw = (unsigned)json_value_get_number(val);
        MSG_DEBUG(DEBUG_INFO, "INFO~ sx1276 transmit power is configured to %u \n", sx1276_pw);
    }
...
int main(void)
{
    ...
    MSG_DEBUG(DEBUG_LOG, "INFO~ sx1276_tx enable:%s, sx1276txpw:%d\n", sx1276_tx ? "yes" : "no", sx1276_pw);
    ...
    if (sx1276_tx) {
        ...
        sxradio->dio[1] = 6;
        sxradio->dio[2] = 8;
        sxradio->rf_power = (sx1276_pw > 10 && sx1276_pw <= 20) ? sx1276_pw : 20; /* sx1276 power < 20 */
