...
static char server_type[16] = "lorawan"; /* Type of server, such as MQTT/LORAWAN/LORA */ 
...
static int keepalive_interval = DEFAULT_KEEPALIVE; /* send a PULL_DATA request every X seconds, negative = disabled */
...
static uint8_t rfsf = 7;            /* tx power of radio */
static uint16_t rfbw = 250;            /* tx power of radio */
static uint8_t rfcr = 4;            /* tx power of radio */
static uint32_t rfprlen = 8;            /* tx power of radio */
static uint8_t rfsyncwd = 52;            /* tx power of radio */
static uint32_t rffreq = 868100000;            /* tx power of radio */
static uint8_t rfpower = 20;            /* tx power of radio */

/* LOG Level */
static uint8_t logdebug = 1;
...
static int parse_radio_configuration(const char * conf_file) {
    ...
    if (conf_obj == NULL) {
        ...
        MSG_DEBUG(DEBUG_INFO, "INFO~ %s does contain a JSON object named %s, parsing radio parameters\n", conf_file, conf_obj_name);
    }

    /* set configuration for RF chains */
    snprintf(param_name, sizeof param_name, "radio_1"); /* compose parameter path inside JSON structure */
    val = json_object_get_value(conf_obj, param_name); /* fetch value (if possible) */
    if (json_value_get_type(val) != JSONObject) {
        MSG_DEBUG(DEBUG_INFO, "INFO~ no configuration for radio 1\n");
        return -1;
    }
    snprintf(param_name, sizeof param_name, "radio_1.freq");
    rffreq = (uint32_t)json_object_dotget_number(conf_obj, param_name);
    snprintf(param_name, sizeof param_name, "radio_1.sf");
    rfsf = (uint32_t)json_object_dotget_number(conf_obj, param_name);
    snprintf(param_name, sizeof param_name, "radio_1.bw");
    rfbw = (uint32_t)json_object_dotget_number(conf_obj, param_name);
    snprintf(param_name, sizeof param_name, "radio_1.cr");
    rfcr = (uint32_t)json_object_dotget_number(conf_obj, param_name);
    snprintf(param_name, sizeof param_name, "radio_1.prlen");
    rfprlen = (uint32_t)json_object_dotget_number(conf_obj, param_name);
    snprintf(param_name, sizeof param_name, "radio_1.syncwd");
    rfsyncwd = (uint32_t)json_object_dotget_number(conf_obj, param_name);
    snprintf(param_name, sizeof param_name, "radio_1.power");
    rfpower = (uint8_t)json_object_dotget_number(conf_obj, param_name);
    MSG_DEBUG(DEBUG_INFO, "INFO~ radio 1 enabled, center frequency %u, sf=%u, bw=%u, cr=%u, prlen=%u, syncwd=%u, power=%u\n", rffreq, rfsf, rfbw, rfcr, rfprlen, rfsyncwd, rfpower);
...
static int parse_gateway_configuration(const char * conf_file) {
    ...
    if (str != NULL) {
        ...
        MSG_DEBUG(DEBUG_INFO, "INFO~ server mode configured to \"%s\"\n", server_type);
    }

    val = json_object_get_value(conf_obj, "logdebug");
    if (val != NULL) {
        logdebug = (uint8_t)json_value_get_number(val);
        MSG_DEBUG(DEBUG_INFO, "INFO~ server log level configured to \"%s\"\n", logdebug);
    }
    ...
    if (val != NULL) {
        keepalive_interval = (int)json_value_get_number(val);
...
int main(int argc, char *argv[])
{
    ...
    int i, x; /* loop variable and temporary variable for return value */
    ...
    /* Set location */
    reference_coord.lat=0.0;
    reference_coord.lon=0.0;
    reference_coord.alt=0;
    ...
    if (access(global_cfg_path, R_OK) == 0) { /* if there is a global conf, parse it and then try to parse local conf  */
        ...
        x = parse_radio_configuration(global_cfg_path);
        if (x != 0) {
            exit(EXIT_FAILURE);
        }
        ...
        if (access(local_cfg_path, R_OK) == 0) {
            ...
            parse_radio_configuration(local_cfg_path);
        ...
        MSG_DEBUG(DEBUG_INFO, "INFO~ found local configuration file %s, parsing it\n", local_cfg_path);
        x = parse_radio_configuration(local_cfg_path);
        if (x != 0) {
            exit(EXIT_FAILURE);
        }
        ...
        MSG_DEBUG(DEBUG_ERROR, "ERROR~ [main] failed to find any configuration file named %s, %s\n", global_cfg_path, local_cfg_path);
        exit(EXIT_FAILURE);
    }
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
    ...
    if (!strcmp(server_type, "lorawan")) {
        MSG_LOG(DEBUG_INFO, "INFO~ Start lora packet forward daemon, server = %s, port = %u\n", server, serv_port_up);
...
void thread_stat(void) {
    ...
    *(uint32_t *)(status_report + 8) = net_mac_l; 
    ...
    while (!exit_sig && !quit_sig) {
        ...
        j = snprintf((char *)(status_report + stat_index), STATUS_SIZE - stat_index, "{\"stat\":{\"time\":\"%s\",\"lati\":%.5f,\"long\":%.5f,\"alti\":%i,\"rxnb\":%u,\"rxok\":%u,\"rxfw\":%u,\"ackr\":%.1f,\"dwnb\":%u,\"txnb\":%u,\"pfrm\":\"%s\",\"mail\":\"%s\",\"desc\":\"%s\"}}", stat_timestamp, reference_coord.lat, reference_coord.lon, (int)reference_coord.alt, cp_nb_rx_rcv, cp_nb_rx_ok, cp_up_pkt_fwd, (float)0, 0, 0, platform, email, description);
        stat_index += j;
...
void thread_down(void) {
    ...
    *(uint32_t *)(buff_req + 8) = net_mac_l;
	...
	recv_time = send_time;
	while ((int)difftimespec(recv_time, send_time) < keepalive_interval) {
